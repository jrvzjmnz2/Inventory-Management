...
router.post('/complete', async (req, res) => {
  try {
    ...
    if (equipmentIds.length > 0 && !EVENT_TYPE_OPTIONS.includes(purpose)) {
      return res.status(400).json({ message: 'Select a valid Event Type before completing the borrow.' });
    }
    if (equipmentIds.length > 0 && !trimmedEvent) {
      return res.status(400).json({ message: 'Enter an Event Name before completing the borrow.' });
    }
